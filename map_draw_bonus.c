...
void	draw_map_2(t_data *data, int j)
{
	...
	if (data->map[data->i][j] == '1')
		mlx_put_image_to_window(data->mlx_ptr, \
			data->mlx_win, data->tree, j * 64, data->i * 64);
	if (data->map[data->i][j] == '0')
		mlx_put_image_to_window(data->mlx_ptr, \
			data->mlx_win, data->ground, j * 64, data->i * 64);
	if (data->map[data->i][j] == 'C')
		mlx_put_image_to_window(data->mlx_ptr, \
			data->mlx_win, data->monster_ball, j * 64, data->i * 64);
	if (data->map[data->i][j] == 'E')
		mlx_put_image_to_window(data->mlx_ptr, \
			data->mlx_win, data->cave, j * 64, data->i * 64);
	if (data->map[data->i][j] == 'P')
		mlx_put_image_to_window(data->mlx_ptr, \
			data->mlx_win, data->trainer, j * 64, data->i * 64);
	if (data->map[data->i][j] == 'F')
		mlx_put_image_to_window(data->mlx_ptr, \
			data->mlx_win, data->rival, j * 64, data->i * 64);
}
